...
def compute_s_target(pred, target):
    return torch.mean(target*pred, axis=1)/torch.mean(target * target, axis=1)*target

def compute_e_interference(pred, inter):
    return torch.mean(inter*pred, axis=1)/torch.mean(inter*inter, axis=1)*inter

def computer_e_artifact(pred, s_target, e_inter):
    return pred - s_target - e_inter

def compute_sdr(s_target, e_inter, e_art):
    error = e_inter + e_art
    return 10*torch.log10(torch.mean(s_target**2, axis=1)/torch.mean(error**2, axis=1))

def compute_sir(s_target, e_inter):
    return 10*torch.log10(torch.mean(s_target**2, axis=1)/torch.mean(e_inter**2, axis=1))

def compute_sar(s_target, e_inter, e_art):
    source_projection = s_target + e_inter
    return 10*torch.log10(torch.mean(source_projection**2, axis=1)/torch.mean(e_art**2, axis=1))
...
def test_metrics():
    ...
    for i in range(len(trainset)):
        ...
        refs = np.stack([sample['target'].numpy(), sample['interference'].numpy()], axis=1)
        ests = np.stack([sample['mixture'].numpy(), sample['interference'].numpy()], axis=1)
        sdrs, sirs, sars = mir_eval.separation.bss_eval_sources(refs, ests)
